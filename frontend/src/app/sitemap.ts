...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const paths = [
    ...
    "/mobile-plans", "/mobile-plans/uk", "/mobile-plans/uk/find", "/tools", "/tools/sms-character-counter",
    "/tools/gsm7-checker", "/tools/sms-segment-calculator", "/tools/unicode-sms-checker",
    "/tools/e164-phone-formatter", "/tools/mcc-mnc-lookup", "/countries", "/calling-codes", "/mcc", "/carriers", "/errors",
  ];
  ...
  const base = paths.map(path => ({ url: `https://signalrate.com${path}`, lastModified: new Date(), changeFrequency: "weekly" as const, priority: path === "" ? 1 : 0.8 }));
  ...
  try {
    ...
    const operators = [...new Map(verified.map(plan => [plan.operator.slug, plan.operator])).values()].map(operator => ({ url: `https://signalrate.com/mobile-plans/uk/${operator.slug}`, lastModified: new Date(), changeFrequency: "weekly" as const, priority: 0.7 }));
    const plans = verified.map(plan => ({ url: `https://signalrate.com/mobile-plans/uk/${plan.operator.slug}/${plan.slug}`, lastModified: plan.last_verified_at ? new Date(plan.last_verified_at) : new Date(), changeFrequency: "weekly" as const, priority: 0.7 }));
  ...
  try{
    ...
    const errorFamilies=families.map(family=>({url:`https://signalrate.com/errors/${family.slug}`,lastModified:new Date(),changeFrequency:"monthly" as const,priority:0.7}));
    const errorPages=errors.map(error=>({url:`https://signalrate.com${error.url}`,lastModified:new Date(error.last_verified_at),changeFrequency:"monthly" as const,priority:0.7}));
  ...
  try{
    ...
    dynamicEntries.push(...countries.map(country=>({url:`https://signalrate.com/countries/${country.slug}`,lastModified:new Date(country.last_verified_at??Date.now()),changeFrequency:"monthly" as const,priority:0.7})));
    ...
    dynamicEntries.push(...mccs.map(mcc=>({url:`https://signalrate.com/mcc/${mcc}`,lastModified:new Date(),changeFrequency:"monthly" as const,priority:0.7})),...verified.map(network=>({url:`https://signalrate.com/mcc/${network.mcc}/${network.mnc}`,lastModified:new Date(network.last_verified_at??Date.now()),changeFrequency:"monthly" as const,priority:0.65})));
    ...
    dynamicEntries.push(...[...new Map(verifiedCarriers.map(operator=>[operator.country.slug,operator.country])).values()].map(country=>({url:`https://signalrate.com/carriers/${country.slug}`,lastModified:new Date(),changeFrequency:"monthly" as const,priority:0.65})),...verifiedCarriers.map(operator=>({url:`https://signalrate.com/carriers/${operator.country.slug}/${operator.slug}`,lastModified:new Date(operator.last_verified_at??Date.now()),changeFrequency:"monthly" as const,priority:0.65})));
  }catch{/* Quality-gated telecom pages are omitted if the directory API is unavailable. */}
  return[...base,...dynamicEntries];
}
